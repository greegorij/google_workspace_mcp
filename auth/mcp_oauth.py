...
# google-mcp jest forkiem samodzielnym (google_workspace_mcp, nie importuje workos_shared),
# więc trzyma wierną kopię. Przy zmianie bramy: edytuj workos_shared, potem zsynchronizuj
# (cp). Kanon: GOLDEN-PATTERNS Wzorzec 9.
...
class McpOAuthGate:
    ...
    def __init__(
        ...
        *,
        ...
        self.redirect_allowlist = frozenset(
            u.strip() for u in redirect_allowlist if u.strip()
        )
        self.google_client_id = google_client_id
        self._google_client_secret = google_client_secret
        self.google_redirect_uri = google_redirect_uri
        self.allowed_emails = frozenset(
            e.strip().lower() for e in allowed_emails if e.strip()
        )
    ...
    def identity_gate_enabled(self) -> bool:
        ...
        return bool(
            ...
            and self._google_client_secret
    ...
    def _cleanup_pending(self) -> None:
        ...
        if len(self._pending_logins) > _MAX_PENDING_LOGINS:
            ...
            for k, _ in sorted(
                self._pending_logins.items(), key=lambda kv: kv[1]["expires_at"]
            )[:excess]:
                del self._pending_logins[k]
    ...
    def _issue_code_and_redirect(self, params: dict):
        ...
        self.log.info(
            "kod autoryzacji wydany, redirect → %s...", params["redirect_uri"][:50]
        )
        query = {"code": code}
        if params.get("state"):
        ...
        return RedirectResponse(
            url=f"{redirect_uri}{sep}{urlencode(query)}", status_code=302
        )
    ...
    async def _verify_google_identity(self, code: str) -> str | None:
        ...
        try:
            ...
            self.log.warning(
                "Google token exchange transport error: %s", type(exc).__name__
            )
            return None
        if resp.status_code != 200:
            self.log.warning(
                "Google token exchange rejected: HTTP %s", resp.status_code
            )
            return None
        ...
        try:
            claims = google_id_token.verify_oauth2_token(
                id_tok, google_requests.Request(), self.google_client_id
            )
        except Exception as exc:  # noqa: BLE001 — weryfikacja podpisu MUSI fail-closed na każdy błąd
            self.log.warning(
                "Google id_token verification failed: %s", type(exc).__name__
            )
            return None
    ...
    async def protected_resource(self, request: Request) -> JSONResponse:
        ...
        base = self._base(request)
        resource = (
            f"{base}{self.resource_path}"
            if request.url.path.endswith(self.resource_path)
            else base
        )
    ...
    async def authorize(self, request: Request):
        ...
        if not self.identity_gate_enabled:
            if self.dev_insecure:
                self.log.warning(
                    "DEV_INSECURE — auto-approve (TYLKO dev, nigdy produkcja)"
                )
                return self._issue_code_and_redirect(params)
            self.log.error(
                "Brama tożsamości nieskonfigurowana — authorize ODMÓWIONY (fail-closed)"
            )
            return JSONResponse(
                {
                    "error": "server_error",
    ...
    async def google_callback(self, request: Request):
        ...
        return self._issue_code_and_redirect(
            {f: pending.get(f, "") for f in _AUTHORIZE_FIELDS}
        )
        ...
        return JSONResponse(
            {
                ...
                "redirect_uris": body.get(
                    "redirect_uris", list(self.redirect_allowlist)
                ),
